...
const BadmintonDetail = () => {
  ...
  const [newMatch, setNewMatch] = useState({
    ...
    useEffect(() => {
    axios.get('http://localhost:9000/api/badminton/list')
        .then(response => {
    ...
    axios.post('http://localhost:9000/api/badminton/create', newMatch)
        .then(response => {
    ...
    axios.post('http://localhost:9000/api/badminton/create-bulk', { matches: bulkMatches })
        .then(response => {
        setSchedules([...schedules, ...response.data]);
  ...
  return (
    ...
    <header className="bg-blue-600 text-white p-4 flex justify-between items-center">
      <h1 className="text-2xl font-bold">Sports Competitions - Badminton</h1>
      <div>
        <button
        ...
        <div className="flex flex-row justify-between">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
            {schedules.map(schedule => (
              <div key={schedule.id} className="bg-white p-6 rounded border shadow-lg hover:shadow-xl transition-shadow">
