...
const SoccerDetail = () => {
  ...
  const [newMatch, setNewMatch] = useState({
    ...
    useEffect(() => {
    axios.get('http://localhost:9000/api/soccer/list')
        .then(response => {
    ...
    axios.post('http://localhost:9000/api/soccer/create', newMatch)
        .then(response => {
    ...
    axios.post('http://localhost:9000/api/soccer/create-bulk', { matches: bulkMatches })
        .then(response => {
        setSchedules([...schedules, ...response.data]);
  ...
  return (
    ...
    <header className="bg-blue-600 text-white p-4 flex justify-between items-center">
      <h1 className="text-2xl font-bold">Sports Competitions - Soccer</h1>
      <div>
        <button
