...
const BasketballDetail = () => {
  ...
  const [newMatch, setNewMatch] = useState({
    ...
    axios.post('http://localhost:9000/api/matches/create', newMatch)
        .then(response => {
    ...
    axios.post('http://localhost:9000/api/matches/create-bulk', { matches: bulkMatches })
        .then(response => {
        setSchedules([...schedules, ...response.data]);
  ...
  return (
    ...
    <header className="bg-blue-600 text-white p-4 flex justify-between items-center">
      <h1 className="text-2xl font-bold">Sports Competitions - Basketball</h1>
      <div>
        <button
      ...
      <main className="flex-1 p-4">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
          {schedules.map(schedule => (
            <a href={`/match/${schedule.id}`} key={schedule.id} className="bg-white p-6 rounded border shadow-lg hover:shadow-xl transition-shadow">
