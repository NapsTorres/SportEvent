...
router.post('/create', async (req, res) => {
  ...
  try {
    const newMatch = await Matches.create({ matchId, team1Id, team2Id, sporttype, date, status, tournamentRoundText });
    res.json(newMatch);
  } catch (err) {
...
router.post('/create-bulk', async (req, res) => {
  ...
  try {
    const newMatches = await Matches.bulkCreate(matches);
    res.json(newMatches);
  } catch (err) {
    console.error(err.message);
    res.status(500).send('Server Error');
